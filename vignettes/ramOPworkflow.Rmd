...
```{r workflow, echo = FALSE, eval = TRUE, fig.align = "center", out.width = "85%"}
knitr::include_graphics(path = "../man/figures/ramOPworkflow.png")
```
...
classicDF <- estimateClassic(x = df, w = testPSU)
```
...
## Perform classic estimation on recoded data using appropriate weights provided by testPSU
classicDF <- estimateClassic(x = df, w = testPSU, replicates = 9)
...
probitDF <- estimateProbit(x = df, w = testPSU)
```
...
## Perform classic estimation on recoded data using appropriate weights provided by testPSU
probitDF <- estimateProbit(x = df, w = testPSU, replicates = 9)
...
resultsDF <- mergeEstimates(x = classicDF, y = probitDF)
...
chartADL(x = create_op_all(testSVY), 
...
knitr::include_graphics(path = paste(tempdir(), "test.ADL.png", sep = "/"))
```
