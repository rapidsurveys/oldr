...
chart_adl(x = create_op(testSVY), save_chart = FALSE)
```
...
The package `magrittr` provides functions that facilitate piped operations in R. The `oldr` package functions were designed in such a way that they can be piped to each other to provide the desired output.
